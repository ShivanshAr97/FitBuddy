...
const Navbar = () => {
  return (
    ...
    <nav className='flex items-center fixed top-0 right-0 z-50 w-full bg-white justify-between p-2 border shadow-lg'>
        <Link to="/home"><h1 className='font-bold text-2xl cursor-pointer text-red-500'>FitBuddy</h1></Link>
        <div className='flex items-center'>
            <ul className='flex'>
                <Link to="/home"><li className='mx-8 cursor-pointer font-semibold'>Home</li>
                </Link>
                <Link to="/bmi">
        ...
        <img className='w-12 h-12 rounded-full cursor-pointer object-cover mx-4' src={profileImg} alt="" />
        </div>
    </nav>
    </>
  )
